        ...
        while true
            ...
            if current_indent <= top_tag.indent
                if top_tag.tag
                    ...
                    if not opt_jinja_tag? or not top.inside? or (top.inside? and opt_jinja_tag not in top.inside) or current_indent < top_tag.indent
...
jinja_tag
    = &{ return options.jinja; } "%" end:"%"? sl:"-"? space? ident:identifier ->
        return { name: ident, spaceless: if sl == "-" then "" else "-" }
